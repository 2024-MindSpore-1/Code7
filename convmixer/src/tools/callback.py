...
from mindspore import Callback, SummaryRecord, RunContext


class BestCheckpointSavingCallback(ms.Callback):
    ...
    def _make_dir(self):
        ...
        if not self.ckpt_dir.exists():
            ...
            print(f'Directory created: {self.ckpt_dir}')
        else:
            print(f'Warning! Directory already exists: {self.ckpt_dir}')
    ...
    def _save_checkpoint(self, network, epoch):
        ...
        if not float('-inf') < self.best_metric < float('inf'):
            return
        ckpt_name = f'ep{epoch}_{self.target_metric}_{self.best_metric:.2f}'
        if self.prefix:
    ...
    def on_eval_end(self, run_context: RunContext):
        ...
        if self.compare(self.best_metric, metrics[self.target_metric]):
            ...
            self._save_checkpoint(cb_params.network, cb_params.cur_epoch_num)


class SummaryCallbackWithEval(ms.SummaryCollector):
    """
    ...
    def on_train_step_begin(self, run_context: RunContext):
        ...
        lr = cb_params.network.optimizer.learning_rate(global_step)
        self._record.add_value('scalar', f'Train/learning_rate', ms.Tensor(lr))
    ...
    def on_eval_end(self, run_context: RunContext):
        ...
        """
        cb_params = run_context.original_args()
        for metric_name, value in cb_params.metrics.items():
            self._record.add_value(
                'scalar', f'Val/{metric_name}', ms.Tensor(value)
            )
