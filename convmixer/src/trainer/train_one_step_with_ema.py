...
# ============================================================================
"""EMA specified TrainOneStep cell."""
...
def tensor_grad_scale_row_tensor(scale, grad):
    return RowTensor(
        grad.indices, grad.values * F.cast(reciprocal(scale),
                                           F.dtype(grad.values)),
        grad.dense_shape
    )
...
class TrainOneStepWithEMA(nn.TrainOneStepWithLossScaleCell):
    ...
    def __init__(
            self, network, optimizer, scale_sense=1.0, with_ema=False,
            ema_decay=0.9999
    ):
        super(TrainOneStepWithEMA, self).__init__(
