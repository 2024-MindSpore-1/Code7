...
def mixup_target(target, num_classes, lam=1., smoothing=0.0):
    ...
    y2 = one_hot(np.flip(target, axis=0), num_classes, on_value=on_value,
                 off_value=off_value)
...
def rand_bbox(img_shape, lam, margin=0., count=None):
    ...
    lam: float
        Cutmix lambda value
    margin: float
        Percentage of bbox dimension to enforce as margin (reduce amount of
        box outside image)
    count: int
...
def rand_bbox_minmax(img_shape, minmax, count=None):
    ...
    based on min/max percent values applied to each dimension of the input
    image.
    ...
    assert len(minmax) == 2
    img_h, img_w = img_shape[-2:]
    cut_h = np.random.randint(int(img_h * minmax[0]), int(img_h * minmax[1]),
                              size=count)
    cut_w = np.random.randint(int(img_w * minmax[0]), int(img_w * minmax[1]),
                              size=count)
...
class Mixup:
    """Mixup/Cutmix that applies different params to elements or whole batch.
        ...
        cutmix_minmax: List[float]
            cutmix min/max image ratio, cutmix is active and uses this vs
            alpha if not None.
        prob: float
            probability of applying mixup or cutmix per batch or element.
        switch_prob: float
            probability of switching to cutmix instead of mixup when both are
            active
        mode: str
            how to apply mixup/cutmix params (per 'batch', 'pair' (pair of
    ...
    def __init__(
        ...
        if self.cutmix_minmax is not None:
            ...
            # force cutmix alpha == 1.0 when minmax active to keep logic
            # simple & safe
    ...
    def _mix_elem(self, x):
            ...
            if lam != 1.:
                if use_cutmix[i]:
                    (yl, yh, xl, xh), lam = cutmix_bbox_and_lam(
                        x[i].shape, lam, ratio_minmax=self.cutmix_minmax,
        ...
        return ms.ops.ExpandDims()(
            ms.Tensor(lam_batch, dtype=mstype.float32), 1
        )
    ...
    def _mix_batch(self, x):
        ...
        if use_cutmix:
            (yl, yh, xl, xh), lam = cutmix_bbox_and_lam(
                x.shape, lam, ratio_minmax=self.cutmix_minmax,
                correct_lam=self.correct_lam
            )
