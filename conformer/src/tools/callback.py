...
def _get_rank_cached() -> Optional[int]:
    try:
        rank = get_rank()
    except RuntimeError:
        rank = None
...
class BestCheckpointSavingCallback(Callback):
    ...
    def _make_dir(self):
        ...
        if not self.ckpt_dir.exists():
            ...
            logging.info('Directory created: %s', str(self.ckpt_dir))
        else:
            logging.warning('Directory already exists: %s', str(self.ckpt_dir))
    ...
    def on_eval_end(self, run_context: RunContext):
        ...
        metrics = cb_params.eval_results
    ...
    def on_train_epoch_end(self, run_context: RunContext):
        ...
        if optimizer is None:
            ...
            if (mlflow is not None and mlflow.active_run() is not None
                    and _get_rank_cached() in [None, 0]):
