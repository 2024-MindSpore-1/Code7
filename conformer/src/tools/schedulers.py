...
def constant_lr(args, batch_num):
    ...
    def _lr_adjuster(epoch):
        if epoch < args.warmup_length:
            lr = _warmup_lr(
                args.warmup_lr, args.base_lr, args.warmup_length, epoch
            )
        else:
            lr = args.base_lr
...
def onecycle_lr(args, batch_num):
    """Get OneCycle lr"""
    learning_rate = []

    assert args.warmup_length == 0, "OneCycle schedule has warmup built in"
    ...
    def _lr_adjuster(epoch):
        lr = np.interp(
            ...
            [0, args.base_lr, args.base_lr / 20.0, 0]
        )[0]
