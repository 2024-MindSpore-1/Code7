...
def preprocess_fn(image, box, is_training):
    ...
    def _data_aug(image, box, is_training):
        ...
        gt_iscrowd_new_revert = (~(gt_iscrowd_new.astype(np.bool_))).astype(np.int32)
...
def data_to_mindrecord_byte_image(dataset="coco", is_training=True, prefix="cascadercnn.mindrecord", file_num=8):
    ...
    writer.add_schema(cascadercnn_json, "cascadercnn_json")
...
def create_cascadercnn_dataset(mindrecord_file, batch_size=2, device_num=1, rank_id=0, is_training=True,
                               num_parallel_workers=8):
    """Create CascadeRcnn dataset with MindDataset."""
    cv2.setNumThreads(0)
